...
class Robot:
    ...
    def derivative_drive(self, drive_meas):
        ...
        if ang_vel == 0:
            ...
            R = lin_vel/ang_vel
            DFx[0, 2] = R*(np.cos(th+dt*ang_vel) - np.cos(th)) # =
            DFx[1, 2] = -R*(-np.sin(th+dt*ang_vel) + np.sin(th))
        return DFx
    ...
    def covariance_drive(self, drive_meas):
        ...
        if ang_vel == 0:
            ...
            Jac2[1, 1] = (lin_vel/(ang_vel**2))*(-np.cos(th) + np.cos(th+dt*ang_vel)) + lin_vel/ang_vel*(dt*np.sin(th2))
