...
def calibrateWheelRadius():
    ...
    for delta_time, wheel_vel in zip(delta_times, wheel_velocities_range):
        scale += 1/(delta_time*wheel_vel)  #: replace with your code to compute the scale parameter using wheel_vel and delta_time
    scale = scale/num
...
def calibrateBaseline(scale):
    ...
    baseline = 0
    for delta_time, wheel_vel in zip(delta_times, wheel_velocities_range):
        baseline += (scale*wheel_vel*delta_time)/np.pi  # : replace with your code to compute the baseline parameter using scale, wheel_vel, and delta_time
